...
from dust3r.utils.image import (
    load_images,
    load_prev_video_results,
    rgb,
    enlarge_seg_masks,
)
from dust3r.utils.device import to_numpy
from dust3r.cloud_opt import global_aligner, GlobalAlignerMode
from dust3r.utils.viz_demo import (
    convert_scene_output_to_glb,
    get_dynamic_mask_from_pairviewer,
)
...
def get_args_parser():
    ...
    parser.add_argument(
        "--prev_output_dir", type=str, default=None, help="previous output dir"
    )
    parser.add_argument(
        "--prev_output_index",
        type=int,
        default=None,
        help="previous output video index",
    )
    parser.add_argument(
        "--silent", action="store_true", default=False, help="silence logs"
    )
    parser.add_argument(
        "--input_dir", type=str, help="Path to input images directory", default=None
    )
    parser.add_argument(
        "--seq_name", type=str, help="Sequence name for evaluation", default="NULL"
    )
    parser.add_argument(
        "--use_gt_davis_masks",
        action="store_true",
    ...
    parser.add_argument(
        "--real_time", action="store_true", default=False, help="Realtime mode"
    )
    parser.add_argument(
        "--window_wise",
        action="store_true",
    ...
    parser.add_argument(
        "--window_overlap_ratio", type=float, default=0.5, help="Window overlap ratio"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Maximum number of frames for video processing",
    )

    # Add "share" argument if you want to make the demo accessible on the public internet
    parser.add_argument(
        "--share", action="store_true", default=False, help="Share the demo"
    )
    return parser
...
def get_reconstructed_scene(
    ...
    if (
        ...
        or scenegraph_type == "swinstride"
        or scenegraph_type == "swin2stride"
    ):
    ...
    output = inference(
        pairs, model, device, batch_size=args.batch_size, verbose=not silent
    )
    # TODO YYJ del model
    if len(imgs) > 2:
    ...
    if mode == GlobalAlignerMode.PointCloudOptimizer:
        if args.window_wise:
            scene.compute_window_wise_alignment(
    ...
    if len(rgbimg) == 2 and rgbimg[0].shape == rgbimg[1].shape:
        ...
        normalized_error_map = (error_map - error_map.min()) / (
...
def set_scenegraph_options(inputfiles, winsize, refid, scenegraph_type):
    ...
    if (
        inputfiles is not None
        and len(inputfiles) == 1
        and inputfiles[0].name.endswith(
...
def get_reconstructed_scene_realtime(
    args,
    model,
    device,
    silent,
    ...
    model.eval()
    imgs = load_images(
        filelist, size=image_size, verbose=not silent, fps=fps, num_frames=num_frames
    )
    if len(imgs) == 1:
        imgs = [imgs[0], copy.deepcopy(imgs[0])]
        imgs[1]["idx"] = 1
    ...
    if scenegraph_type == "oneref":
        ...
        raise ValueError(
            f"Unknown scenegraph type for realtime mode: {scenegraph_type}"
        )

    pairs = make_pairs(
        imgs, scene_graph=scenegraph_type, prefilter=None, symmetrize=False
    )
    output = inference(
        pairs, model, device, batch_size=args.batch_size, verbose=not silent
    )
...
def main_demo(
    ...
    recon_fun = functools.partial(
        get_reconstructed_scene, args, tmpdirname, model, device, silent, image_size
    )
        ...
        with gradio.Column():
            ...
            with gradio.Row():
                ...
                min_conf_thr = gradio.Slider(
                    label="min_conf_thr", value=1.1, minimum=0.0, maximum=20, step=0.01
                )
                ...
                temporal_smoothing_weight = gradio.Slider(
                    label="temporal_smoothing_weight",
                    value=0.01,
                    minimum=0.0,
                    maximum=0.1,
                    step=0.001,
                ...
                translation_weight = gradio.Textbox(
                    label="translation_weight",
                    placeholder="1.0",
                    value="1.0",
                    info="For evaluation",
                )
                # change to another model
                new_model_weights = gradio.Textbox(
                    label="New Model",
                    placeholder=args.weights,
                    value=args.weights,
                    info="Path to updated model weights",
                )
            with gradio.Row():
                ...
                use_davis_gt_mask = gradio.Checkbox(
                    value=False, label="Use GT Mask (DAVIS)"
                )
            with gradio.Row():
                flow_loss_weight = gradio.Slider(
                ...
                flow_loss_start_iter = gradio.Slider(
                    label="Flow Loss Start Iter",
                    value=0.1,
                    minimum=0,
                    maximum=1,
                    step=0.01,
                )
                flow_loss_threshold = gradio.Slider(
                    label="Flow Loss Threshold",
                    value=25,
                    minimum=0,
                    maximum=100,
                    step=1,
                ...
                num_frames = gradio.Slider(
                    label="Num Frames", value=100, minimum=0, maximum=200, step=1
                )

            outmodel = gradio.Model3D()
            outgallery = gradio.Gallery(
                label="rgb,depth,confidence, init_conf", columns=4, height="100%"
            )
...
if __name__ == "__main__":
    ...
    tmpdirname = (
        args.output_dir
        if args.output_dir is not None
        else tempfile.mkdtemp(suffix="monst3r_gradio_demo")
    )
    ...
    if args.input_dir is not None:
        ...
        if args.real_time:
            recon_fun = functools.partial(
                get_reconstructed_scene_realtime,
                args,
                model,
                args.device,
                args.silent,
